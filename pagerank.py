...
max_node_id. The algorithm uses power iteration method iterating over
probabilities because computing the adjacency matrix for large graph requires
loading large graph dataset into computer memory. In order to calculate the
probabilities of each node, it iterates over dataset multiple times and update
the probabilities.
"""
...
class PageRank:
    ...
    The out-degree in class variable is stored in "node_degree" and maximum
    node id in "max_node_id".
    """
    ...
    def run_pagerank(self, node_weights,  damping_factor=0.85, iterations=10):
        ...
        The score of each node is obtained by using the calculated out-degree.

        Args:
            node_weights (float): probability of each node to flyout during
              random walk.
            damping_factor (float): probability of continuing on the random
              walk.
            iterations (int): number of iterations to run the algorithm.

        Returns:
            scores (float): the score of each node.
        """
        ...
        for it in range(iterations):
            ...
            for source, target in self.read_edge_file(self.edge_file):
                new_scores[target] += scores[source]/self.node_degree[source]
            for i in range(len(new_scores)):
                new_scores[i] = (1 - damping_factor)*node_weights[i] + damping_factor*new_scores[i]
            scores = new_scores
...
def print_results(command, iterations, result):
...
if __name__ == "__main__":
    ...
    if args.command == 'simplified_pagerank':
        ...
        print_results(args.command, args.iterations, result)
